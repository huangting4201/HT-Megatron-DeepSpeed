...
class ParallelMLP(MegatronModule):
    ...
    def forward(self, x):
        w1_o, _ = self.dense_h_to_4h(x)
        w2_o, _ = self.dense_h_to_4h_w2(x)
        out, out_bias = self.dense_4h_to_h(F.silu(w1_o) * w2_o)
...
class FlashSelfAttention(torch.nn.Module):
    ...
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0, device=None, dtype=None):
        super().__init__()
        assert (
            flash_attn_unpadded_func is not None or flash_attn_varlen_func is not None or flash_attn_builder is not None
    ...
    def forward(self, q, k, v):
        ...
        assert all((i.dtype in [torch.bfloat16] for i in (q, k, v)))
        ...
        if get_accelerator().device_name() == "cuda":
            ...
            cu_seqlens_q = torch.arange(
                0, (batch_size + 1) * seqlen_q, step=q.shape[0], dtype=torch.int32, device=q.device
            )
            seqlen_q = q.shape[0]
            seqlen_k = k.shape[0]
        else:
        ...
        if self.training:
            ...
            cu_seqlens_k = (
                torch.arange(0, (batch_size + 1) * seqlen_k, step=seqlen_k, dtype=torch.int32, device=q.device)
                if get_accelerator().device_name() == "cuda"
                else None
            )
            self.dropout_p = 0
        ...
        output = (
            ...
            if get_accelerator().device_name() == "cuda"
            else flash_attn_builder.flash_attn_func(q, k, v, self.dropout_p, self.softmax_scale, is_causal)
        )

        output = (
            rearrange(output, "(b s) ... -> b s ...", b=batch_size)
            if get_accelerator().device_name() == "cuda"
            else rearrange(output, "b h s d -> b s h d").contiguous()
        )
        return output
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(
        ...
        if args.normalization == "layernorm":
            if get_accelerator().device_name() == "cuda":
                ...
                self.input_layernorm = LayerNorm(config.hidden_size, eps=config.layernorm_epsilon)
        else:
        ...
        if args.normalization == "layernorm":
            if get_accelerator().device_name() == "cuda":
                ...
                self.post_attention_layernorm = LayerNorm(config.hidden_size, eps=config.layernorm_epsilon)
        else:
    ...
    def default_decoder_cross_attention(
        self, encoder_output, enc_dec_attn_mask, layernorm_input, layernorm_output, bias_dropout_add_func
    ):
    ...
    def retro_encoder_cross_attention(self, retriever_output, layernorm_input, layernorm_output, bias_dropout_add_func):
        ...
        chunked_outputs = layernorm_output.reshape(self.retro_retrieved_length, -1, self.retro_num_neighbors, d)
    ...
    def retro_decoder_cross_attention(
        ...
        with torch.enable_grad():
            ...
            layernorm_input = layernorm_input.reshape(self.retro_chunk_length, bs, l, d).permute(
                2, 0, 1, 3
            )  # [l, m, bs, d]
            layernorm_input = layernorm_input.reshape(self.retro_chunk_length * l, bs, d)
            layernorm_input = torch.nn.functional.pad(layernorm_input, (0, 0, 0, 0, pad, 0), "constant", 0)[
                :ns
            ]  # [ns, b, d]
    ...
    def forward(
        ...
        if self.layer_type == LayerType.encoder:
            ...
            layernorm_input, layernorm_output = self.default_decoder_cross_attention(
                encoder_output, enc_dec_attn_mask, layernorm_input, layernorm_output, bias_dropout_add_func
            )
        elif self.layer_type == LayerType.retro_encoder:
            layernorm_input, layernorm_output = self.retro_encoder_cross_attention(
                retriever_output, layernorm_input, layernorm_output, bias_dropout_add_func
            )
        elif self.layer_type in (LayerType.retro_decoder, LayerType.retro_decoder_with_retriever):
            retriever_output, layernorm_input, layernorm_output = self.retro_decoder_cross_attention(
                retriever_input,
                retriever_output,
                retriever_attn_mask,
                layernorm_input,
    ...
    if model_type == ModelType.retro_encoder:
        ...
        if is_encoder_and_decoder_model:
            ...
            if parallel_state.is_pipeline_stage_before_split():
                num_layers = (
                    0
                    if args.standalone_embedding_stage and parallel_state.get_pipeline_model_parallel_rank() == 0
