...
def pretrain(
    ...
    if args.deepspeed:
        ...
        if (
            ...
            args.curriculum_learning_legacy = args.deepspeed_config_dict["curriculum_learning"]["enabled"]
        if args.curriculum_learning_legacy and not args.no_pipeline_parallel:
            from deepspeed.runtime.data_pipeline.curriculum_scheduler import CurriculumScheduler

            args.curriculum_scheduler = CurriculumScheduler(args.deepspeed_config_dict["curriculum_learning"])
...
def load_model_weights_only(model_provider_func):
    ...
    if args.deepspeed:
        ...
        model, optimizer, _, lr_scheduler = deepspeed.initialize(model=model[0], config=args.deepspeed_config_dict)
    ...
    if args.deepspeed:
        ...
        if args.data_efficiency_curriculum_learning and build_train_valid_test_datasets_provider is not None:
            ...
            if mpu.get_tensor_model_parallel_rank() == 0:
                ...
                # Build the datasets.
                train_ds, _, _ = build_train_valid_test_datasets_provider(train_val_test_num_samples)
...
def training_log(
    ...
    for key in loss_dict:
        if not skipped_iter:
            total_loss_dict[key] = total_loss_dict.get(key, get_accelerator().FloatTensor([0.0])) + loss_dict[key]
        else:
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        if args.log_batch_size_to_tensorboard:
            ...
            writer.add_scalar("batch-size/batch-size vs samples", batch_size, args.consumed_train_samples)
            writer.add_scalar("batch-size/batch-size vs tokens", batch_size, args.consumed_train_tokens)
        for key in loss_dict:
        ...
        if args.log_world_size_to_tensorboard:
            ...
            writer.add_scalar("world-size/world-size vs samples", args.world_size, args.consumed_train_samples)
            writer.add_scalar("world-size/world-size vs tokens", args.world_size, args.consumed_train_tokens)
        ...
        if args.curriculum_learning_legacy or args.data_efficiency_curriculum_learning:
            ...
            writer.add_scalar(
                "seqlen/curriculum_seqlen vs samples", args.curriculum_seqlen, args.consumed_train_samples
            )
            writer.add_scalar("seqlen/curriculum_seqlen vs tokens", args.curriculum_seqlen, args.consumed_train_tokens)
        if args.random_ltd:
            writer.add_scalar("seqlen/random_ltd_reserved_length", args.random_ltd_reserved_length, iteration)
            writer.add_scalar(
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            for _, group in enumerate(optimizer.param_groups):
                for _, param in enumerate(group["params"]):
                    ...
                    opt_stats_2[1] = max(
                        opt_stats_2[1], optimizer.state[param]["exp_avg_sq"].sqrt().abs_().max().item()
                    )
                    opt_stats_2[2] = max(
                        opt_stats_2[2],
            ...
            if args.zero_stage > 0:
                ...
                torch.distributed.all_reduce(
                    opt_stats_2, op=torch.distributed.ReduceOp.MAX, group=mpu.get_sequence_data_parallel_group()
                )
            ...
            if args.tensor_model_parallel_size > 1:
                ...
                opt_stats_2 = get_accelerator().FloatTensor(opt_stats_2)
                torch.distributed.all_reduce(
                    opt_stats_2, op=torch.distributed.ReduceOp.MAX, group=mpu.get_tensor_model_parallel_group()
                )
            ...
            if args.pipeline_model_parallel_size > 1:
                ...
                opt_stats_2 = get_accelerator().FloatTensor(opt_stats_2)
                torch.distributed.all_reduce(
                    opt_stats_2, op=torch.distributed.ReduceOp.MAX, group=mpu.get_pipeline_model_parallel_group()
                )
            ...
            if writer and is_last_rank():
                ...
                writer.add_scalar(
                    "optimizer/variance_sqrt_l2 vs tokens", opt_stats[1] ** 0.5, args.consumed_train_tokens
                )
                ...
                writer.add_scalar(
                    "optimizer/variance_sqrt_abs_max vs tokens", opt_stats_2[1], args.consumed_train_tokens
                )
    ...
    if iteration % args.log_interval == 0:
        ...
        if writer:
            if args.log_timers_to_tensorboard:
                ...
                writer.add_scalar(
                    "iteration-time/iteration-time vs samples", elapsed_time_per_iteration, args.consumed_train_samples
                )
                writer.add_scalar(
                    "iteration-time/iteration-time vs tokens", elapsed_time_per_iteration, args.consumed_train_tokens
                )
...
def train(
    ...
    with llm_profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler(f"traces/rank_{torch.distributed.get_rank()}"),
        with_stack=True,
        with_modules=True,
        profile_memory=True,
    ) as prof:
        while iteration < args.train_iters and (
            ...
            if args.curriculum_learning_legacy and not args.no_pipeline_parallel:
                curriculum_seqlen = args.curriculum_scheduler.update_difficulty(args.iteration + 1)
...
def evaluate_and_print_results(
    ...
    for key in total_loss_dict:
        ...
        if writer and is_last_rank():
            ...
            writer.add_scalar(f"lm-loss-validation/{key} {data_type}", total_loss_dict[key].item(), iteration)
            writer.add_scalar(
            ...
            if args.log_validation_ppl_to_tensorboard:
                ...
                writer.add_scalar(
                    f"lm-loss-validation/{key} {data_type} ppl vs samples", ppl, args.consumed_train_samples
                )
                writer.add_scalar(
                    f"lm-loss-validation/{key} {data_type} ppl vs tokens", ppl, args.consumed_train_tokens
                )
...
def build_train_valid_test_data_loaders(build_train_valid_test_datasets_provider):
    ...
    rank_in_parallel_group = (
        mpu.get_sequence_parallel_rank() if ds_sequence_parallel else mpu.get_tensor_model_parallel_rank()
    )
    ...
    if ds_sequence_parallel:
        torch.distributed.broadcast(
            flags, mpu.get_sequence_parallel_src_rank(), group=mpu.get_sequence_parallel_group()
        )
    else:
        torch.distributed.broadcast(
