    ...
    with deepspeed.zero.Init(
        ...
        if args.deepspeed and not args.no_pipeline_parallel:
            ...
            attention_mask = torch.tril(
                torch.ones((1, args.seq_length, args.seq_length), device=get_accelerator().current_device_name())
...
def get_batch(data_iterator):
    ...
    # append_value = torch.tensor([-100]).to(labels.device)
    # new_tensor = torch.cat((tensor, append_value))

    # labels = torch.cat((labels, append_value))
    # labels=labels.append(-100)
    ...
    attention_mask, loss_mask, position_ids = get_ltor_masks_and_position_ids(
        tokens, tokenizer.eod, args.reset_position_ids, args.reset_attention_mask, args.eod_mask_loss, skip_mask
    )
    ...
    # print(f'seq_parallel_world_size{seq_parallel_world_size}')
...
if __name__ == "__main__":
    git_ds_info()
    pretrain(
        random_train_valid_test_datasets_provider,
        model_provider,
